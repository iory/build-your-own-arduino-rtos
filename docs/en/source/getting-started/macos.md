...
macOS creates two device nodes for the same board:
    ...
    /dev/cu.usbmodemXXXXXXXXXXXXX      ← use this one
    /dev/tty.usbmodemXXXXXXXXXXXXX
...
```bash
uv run pio run -d 01_boot -t upload --upload-port /dev/cu.usbmodemXXXXXXXXXXXXX
```
...
```text
port: /dev/cu.usbmodemXXXXXXXXXXXXX
ps
