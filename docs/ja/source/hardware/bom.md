...
組み立てに使うネジ（M3×6 ×40、M2×6 ×16）は **STS3215 の付属品で足りる**ので、
別途買わなくてよい。
...
|---|---|---|---|---|
| 胴体 | `body.stl` | **230.0 × 110.0 × 50.0** | 1 | 244.3 cm³ |
...
このページで配布している **STL・3MF・CAD・URDF・ファームウェア・学習済み歩行ポリシー**は
[Apache License 2.0](https://github.com/iory/build-your-own-arduino-rtos/blob/main/LICENSE) です。

- **商用利用できます。** キット化して販売しても構いません。事前の許諾も連絡も不要です。
- 著者および東京大学から、ライセンス料・実施料を請求することはありません。
- 独占的な取り扱いをお願いすることもありません。誰でも作れて、誰でも売れます。
- 改変・再配布も自由です。Apache-2.0 の条件（著作権表示と LICENSE の同梱、
  変更点の明示）に従ってください。

商標はライセンスの対象外です。「本書対応」「SO-101 互換」のような**事実の記載は自由**ですが、
「公式」「東京大学」「著者監修」といった表示は、別途ご相談ください。

```{admonition} キットを販売される事業者の方へ
:class: tip
販売を始めたら [Issue](https://github.com/iory/build-your-own-arduino-rtos/issues) か
メールでお知らせください。このページの「キットで買う」欄に掲載します。掲載は無償で、
掲載料も紹介料もいただきません。実機で動作確認できたものには、その旨を併記します。
```
