...
```{note}
STL データは**公開準備中**です。公開までにサイズを確認したい場合は下の外形寸法を
参照してください。
```

| 部品 | 外形 [mm] | 個数 | 実体積 |
|---|---|---|---|
| 胴体 | **250.0 × 110.0 × 50.0** | 1 | 239.8 cm³ |
| 腿 | 74.3 × 52.0 × 61.2 | 4 | 42.9 cm³ |
| 足先 | 105.7 × 12.0 × 91.5 | 4 | 17.5 cm³ |

**9 部品、実体積の合計 481 cm³。**

**腿は 4 本とも同じ形。** 鏡像ですらない。**1 つの STL を 4 回刷ればよく、
左右の作り分けは要らない。** 足先も同じ。
...
シミュレーションモデル（MJCF）の質量を実体積で割ると、3 部品とも **1.24 g/cm³**
ちょうどになる。
...
- スライサの数値は Bambu Studio 02.06.00.51 の `profiles/BBL/`（`filament/` と
  `process/` の `inherits` を解決して読み出したもの）
- [DMM.make 3Dプリント](https://make.dmm.com/print/)（素材別 1 cm³ 単価、造形サイズ）
- 寸法・体積・質量は STL とロボットモデル（MJCF）の実測
