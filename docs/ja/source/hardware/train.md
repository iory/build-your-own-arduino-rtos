...
（home 高さ 0.11 m、歩容周期 0.32 s、3999 iteration、指令 0.12 m/s で 0.120 m/s）の再現では
...
mjlab 版（`rl/scripts/train.sh`）はそのまま残っています。両者は同じタスクで、
チェックポイントの形式も同じなので、**どちらで学習したものもどちらで続きから
学習でき、同じエクスポータでヘッダに書き出せます**。CPU 版のチェックポイントを
書き出すときは `--backend cpu` を付けます。
...
```{note}
画面の無いサーバで `evaluate.py --video` を使うときは `MUJOCO_GL=egl` を付けて
ください。付けないと MuJoCo が GL のコンテキストを作れず、`Renderer` の生成で
落ちます。
```

### どれくらいかかるか

ノート PC（Core i9-8950HK、6 コア 12 スレッド、2018 年）での実測です。
...
同じくらいです。同じノート PC で 2048 環境 × 2300 iteration を通しで回すと
**68.9 分**でした（1.80 秒/iteration）。上の表の 1.55 秒はベンチだけを測った値で、
実際の学習ではログとチェックポイントの書き出しが乗ります。
...
```{note}
CPU 版が mjlab 版と同じタスクになっていることは `rl/cpu/parity_check.py` で
確認しています（同じ初期状態・同じ行動列で両方を回し、関節角・観測・報酬 26 項を
ステップごとに比較）。検査のやり方と、残っている差は `rl/cpu/README.md` にあります。
```
