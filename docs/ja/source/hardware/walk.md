...
シミュレーションでの性能: 前進指令 0.12 m/s で 0.165 m/s（0.66 体長/秒）、
最大の指令 0.15 m/s で 0.20 m/s。指令より 4 割ほど速く歩きます。対角トロットは
約 3.1 Hz（歩容周期 0.32 s）。いずれも上のシミュレーションと同じモデルで 20 秒
歩かせ、後半 10 秒の平均を取った値です。遅いのはサーボの無負荷回転数で決まる
機体の性質で、学習の失敗ではありません。
...
0.05〜0.2 m/s が素の実力です（シミュレーションでは最大の指令で 0.20 m/s）。律速はトルクではなく無負荷回転数なので、
