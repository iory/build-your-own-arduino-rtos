...
CAD アセンブリから**自動生成**した組み立て手順です。全 9 手順（ユニット内部手順を
含む）が**直線挿入の掃引検証を通過**しており、この順序で幾何的に組み立てられる
ことを確認済みです。

ネジのサイズ・本数は CAD のネジ部品（ISO 7045）から取っていますが、
**長さ・ワッシャ有無は実物で確認**してください。
...
```{warning}
脚をサーボのホーンに固定する前に、**サーボを必ずキャリブレーション（原点合わせ）**
してください。ホーンの取り付け角がずれたまま組むと、歩行姿勢が出ません。
手順は第13章を参照。
```
...
```{note}
この手順書は SolidWorks アセンブリの export（sw2robot）から
[create-assembly-view](https://github.com/iory/create-assembly-view) で自動生成
したものです。全ステップは干渉チェック（直線挿入の掃引検証）済み。
```
