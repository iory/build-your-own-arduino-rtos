...
PC に何も入れずに、LED の点滅やシェルを触れます。自分で書き換えてビルドした
`firmware.elf` も動かせます（{ref}`browser-sim-elf`）。
...
ページを開くと、各章のファームウェア（`pio run -e sim` でビルドしたもの）が
ブラウザの中の QEMU で動き出します。章は上の一覧で切り替えます。シェルのある章は、
画面下の入力欄にコマンドを打てばそのまま動きます。

(browser-sim-elf)=
### 自分でビルドした ELF を動かす

サンプルを書き換えたら、ビルドしてできた `firmware.elf` をブラウザ版に渡すだけで、
書き込まずに動きを確かめられます。ここでは `00_intro` のメッセージを書き換えてみます。

1. `00_intro/src/main.cpp` の `Environment OK!` を、好きな文に書き換えます
   （下の GIF では `sed` で `Hello from my build!` に置き換えています）。
2. ビルドします。書き込み（`-t upload`）は要りません。

   ```bash
   pio run -d 00_intro
   ```

   できた ELF は `00_intro/.pio/build/uno_r4_wifi/firmware.elf` にあります。

   ```{figure} ../_static/sim_upload_build.gif
   :width: 100%

   メッセージを書き換えてビルドし、`firmware.elf` ができたところ
   ```

3. <a href="../sim/index.html">ブラウザ版</a>を開き、章の一覧の最後の
   **「自分でビルドした ELF を動かす…」** を選びます。
4. 出てきた欄に `firmware.elf` をドロップします（欄をクリックしてファイルを選んでも同じです）。
5. Serial monitor に書き換えたメッセージが出て、LED マトリクスに「OS」が出れば成功です。

   ```{figure} ../_static/sim_upload_run.gif
   :width: 80%

   ビルドした `firmware.elf` をドロップすると、書き換えたメッセージが出る
   ```

:::{note}
- **UNO R4 WiFi 向けのふつうのビルドがそのまま動きます。** Arduino の
  `boards.txt` は UNO R4 WiFi に `-DNO_USB` を付けていて、`Serial` は元から
  UART です（`pio run` と `pio run -e sim` の ELF はまったく同じものになります）。
- ファイルはブラウザの中だけで使われ、どこにも送られません。
- 選べるのは `.elf` だけです（`.bin` や `.hex` は使えません）。UNO R4 Minima 向け
  のように `Serial` を USB で出すビルドは、エミュレータに USB が無いのでシリアルが
  出ません（ページに注意が出ます）。
- 章を切り替えるとページが読み込み直されます。動かし直すときは、もう一度
  「自分でビルドした ELF を動かす…」を選んでください。
:::

### ブラウザ版の制限

- **速さは実機の約 1/10 です。** タイマや点滅は実時間どおりに進みますが、重い計算は
  遅くなります。第4章の Heavy タスクはほとんど進まず、第9章の `ps` の CPU% は
  合計が 100% を少し超えることがあります（ページにも注意が出ます）。
- **起動が終わる前に打った文字は失われます。** プロンプト（`>` など）が出てから
  打ってください。

本文の出力と細かく比べるときは、以下の手順で PC に入れたものか実機を使ってください。
