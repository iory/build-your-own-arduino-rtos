...
~~~{admonition} 第1章が `S` を待つ理由
:class: note

第1章のスケッチは、モニタから **`S` を 1 文字送るまで印字を始めません**。
モニタが開いたら `S` を打ってください。
...
**番号は固定ではありません。** 抜き挿ししたり、ほかの USB シリアル機器を
先につないだりすると、同じボードが `COM3` から `COM5` に変わることがあります。
急に `FileNotFoundError` が出たら、まずこれを疑ってください。
デバイスマネージャーの「ポート (COM と LPT)」でも確認できます。

Linux の `/dev/ttyACM0` / `/dev/ttyACM1` でも同じことが起きます。
番号に依存しない書き方は「6. Python からボードと通信する」にあります。
...
{doc}`第5章（05_shell）<../chapters/ch05>` を書き込んだ状態で実行します。
