...
本文の C/C++ コードブロックが付属コードに実在するかの照合は、
`docs/os-on-arduino/scripts/check_code_sync.py` が担当する（CI は `code-sync.yml`）。
例外は同スクリプトの `EXEMPT` に理由付きで登録する。

```bash
python3 docs/os-on-arduino/scripts/check_code_sync.py
```

## 旧スクリプト（Linux 専用）

`flash_and_capture.py` と `run_all_chapters.py` は agentbox1（Ubuntu）専用で、
